...
public class Rope : MonoBehaviour {
    ...
    void OnEnable ()
    {
        GetComponent<RemoveRope>().enabled = false;

        if (parentLine == null)
            return;
        myLine = parentLine.GetChild(0).GetComponent<LineRenderer>();
        List<Vector3> linePos = new List<Vector3>();
        for (int i = 0; i < myLine.positionCount; i++)
        {
            linePos.Add(myLine.GetPosition(i));
        }
        nodes = linePos;
        ...
        GetComponent<RemoveRope>().enabled = true;
    ...
    private static Rigidbody2D GetConnectedObject(Vector2 position, Rigidbody2D originalObj)
    {
