...
public class SoundManager : Singleton<SoundManager>
{
    ...
    public void ChangingAudioVolume(float volume)
    {
        ...
        else
        {
            ...
            {
                transform.GetChild(i).gameObject.GetComponent<AudioSource>().volume = volume;
                if(transform.GetChild(i).name == SoundType.BGMain.ToString() && !transform.GetChild(i).gameObject.activeInHierarchy)
                {
    ...
    public void StopRopePullingSound()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            if (transform.GetChild(i).name.Contains("PullRope"))
            {
                transform.GetChild(i).gameObject.SetActive(false);
            }
        }
